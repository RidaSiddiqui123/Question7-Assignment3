   ...
   for d in degree: #iterates over degree
       for k in kernel: #iterates kernel
           for dfs in decision_function_shape: #iterates over decision_function_shape
                ...
                #--> add your Python code here
                clf = svm.SVC(C = num, degree = d, kernel = k, decision_function_shape = dfs)
                #clf = svm.SVC()
                ...
                curr_accuracy = correctPrediction/len(X_test)

                # check if the calculated accuracy is higher than the previously one calculated. If so, update the highest accuracy and print it together
                # with the SVM hyperparameters. Example: "Highest SVM accuracy so far: 0.92, Parameters: a=1, degree=2, kernel= poly, decision_function_shape = 'ovo'"
                # --> add your Python code here
                if curr_accuracy > Accuracy:
                    print("Highest SVM accuracy so far: " + str(curr_accuracy) + ", Parameters: c=" + str(num) + ", degree=" + str(d) + ", kernel=" + str(k) + ", decision_function_shape=" + str(dfs))
